...
Three sign-in paths are supported: a six-digit email OTP (`signInWithOtp` then `verifyOtp`), a magic link landing on `/auth/callback`, and email/password. The OTP and magic-link paths both use `shouldCreateUser: true`, so signup and sign-in are one step. The Magic Link email template must include `{{ .Token }}` or the six-digit code never reaches the inbox — Supabase ships that template with only `{{ .ConfirmationURL }}`. Both `/auth/callback` and the site URL must be on the Auth redirect allowlist. The callback route accepts only same-origin relative redirect targets.
